...
typedef struct {
   opttype_t type;          // rule condition coding
     QString alias;
...
class MyPlcDevice
{
    ...
    MyPlcDevice(QObject *parent = nullptr);
    ~MyPlcDevice();

 public:

 optdata_t *getOpt(const int iopt);

      // Add new Option
       int addOpt(const optdata_t &opt);
       int addOpt(const vs_module_t &mod);
       int addOpt(const vs_rule_t &rule);

      // Delete Option
      void delOpt(const int iopt);

      // Update config's Opt
      void updInf(vs_info_t *info);
      void updOpt(const int iopt, const optdata_t &opt);

      void updModule(vs_module_t module);
      void updPort(vs_port_t port);
      void updRule(vs_rule_t  rule);
...
//Q_DECLARE_METATYPE(vs_module_t);
//Q_DECLARE_METATYPE(vs_port_t);
//Q_DECLARE_METATYPE(vs_rule_t);
...
Q_DECLARE_METATYPE(itemmeta_t);





Q_DECLARE_METATYPE(MyPlcDevice);
